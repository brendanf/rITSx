...
TF <- function(x) {
  ...
  #valid argument
  assertthat::assert_that(is.flag(x))
}
...
#'
#'
#' @param in.file Name of a fasta file to read sequences from. Alternatively, a member of classes \link[ShortRead]{ShortRead}, \link[Biostrings]{DNAStringSet} or \link[seqinr]{SeqFastadna}, in which case the sequences will be written to a temporary .fasta file.
#' @param out.root Root name for all output files.  The default will put these in a pseudorandomly generated file name in the R temp directory, which is probably not what you want if you intend to access them later.
#' @param taxon The taxonomic group(s) to attempt to find rDNA from.  See the manual for ITSx for available options.  In contrast to the default for ITSx ("all") the default for this function is "fungi".
#' @param Evalue E-value cutoff for inclusion in results.
#' @param Svalue Score cutoff for inclusion in results.
#' @param Nvalue Number of domains required for inclusion in results.
...
#' @param summary Output summary results in \code{[out.root].summary.txt}.
#' @param graphical Output graphical results in \code{[out.root].graph}.
#' @param fasta Output full ITS (ITS1 + 5.8S + ITS2) results in \code{[out.root].full.fasta}.
#' @param preserve Use original sequence headers instead of writing new ones.
#' @param save_regions Additional reagions to save output for; Options are one or more of "SSU", "ITS1", "5.8S", "ITS2", "LSU", or "all" or "none". There will be output in \code{[out.root].SSU.fasta}, \code{[out.root].ITS1.fasta}, etc.
#' @param anchor Number of extra bases included at the beginning and end of each region.
#' Only one of \code{anchor} and \code{require_anchor} may be given.
#' @param require_anchor As \code{anchor}, but the anchor bases are required to be present for the region to be included in output.
#' Only one of \code{anchor} and \code{require_anchor} may be given.
#' @param only_full Limit output to full length ITS1 and ITS2 regions.
#' @param partial Save additional files for partial regions.  The argument give the minimum number of bases required.  These will be saved as \code{[out.root].full_and_partial.fasta} for the full ITS region, and as \code{[out.root].SSU.full_and_partial.fasta}, \code{[out.root].ITS1.full_and_partial.fasta}, etc. for the other regions.
#' @param concat Output concatenated ITS1 and ITS2 regions in \code{[out.root].concat.fasta}.
#' @param minlen Minimum length for ITS regions to be included in \code{concat}.
#' @param positions Output a table of positions where HMMER matches were detected in \code{[out.root].positions.txt}.
#' @param table Output a table of ITS results in \code{[out.root].hmmer.table}.
#' @param detailed_results Output detailed results in \code{[out.root].extraction.results}.
#' @param not_found  Output a list of sequences for which no match was found in \code{[out.root]_no_detections.txt}.
#' @param truncate Remove ends of ITS sequences if they extend beyond the ITS region.
...
itsx <- function(in.file, out.root = tempfile("itsx"),
  ...
  save_regions <- match.arg(save_regions,
                            c("ITS1", "ITS2", "SSU", "LSU", "5.8S", "all", "none"),
                            several.ok = TRUE)

  assertthat::assert_that(
    assertthat::is.number(Evalue),
    assertthat::is.count(Svalue) ||
      assertthat::is.number(Svalue) && Svalue == 0,
    assertthat::is.count(Nvalue) ||
      assertthat::is.number(Svalue) && Svalue == 0,
    assertthat::is.string(selection_priority),
    missing(search_score) || missing(search_eval),
    missing(search_eval) ||
      (assertthat::is.number(search_eval) && search_eval >= Evalue),
    missing(search_score) ||
      (assertthat::is.count(search_score) && search_score <= Svalue),
  ...
  if (methods::is(in.file, "ShortRead")) {
    .data <- in.file
    in.file <- paste0(out.root, ".fasta")
    ShortRead::writeFasta(.data, in.file = paste0(out.root, ".fasta"))
    on.exit(file.remove(in.file))
  } else if (methods::is(in.file, "DNAStringSet")) {
    .data <- in.file
    in.file <- paste0(out.root, ".fasta")
    Biostrings::writeXStringSet(.data, in.file)
    on.exit(file.remove(in.file))
  } else if (is.list(in.file) &&
    ...
    assertthat::assert_that(file.exists(in.file),
                            file.access(in.file, 4) == 0)
  }

  #required command line
  cl <- glue::glue('ITSx -i "{in.file}" -o "{out.root}"',
                   '-t "{paste(taxon, collapse = ",")}"',
                   '-E {format(Evalue, scientific = TRUE)}',
                   '-S {Svalue} -N {Nvalue}',
...
itsx_read_output <-  function(out.root, summary = TRUE,
                              ...
                              read_function) {
  out <- list()
  do.partial <- partial > 0
  if (isTRUE(fasta)) {
    readfile <- paste0(out.root, ".full.fasta")
    ...
    if (isTRUE(do.partial)) {
      readfile <- paste0(out.root, ".full_and_partial.fasta")
      if (file.exists(readfile)) {
        out$full.partial <- read_function(readfile)
        file.remove(readfile)
      } else {
        out$full.partial <- NULL
      }
    }
  }

  if ("all" %in% save_regions) {
    out.regions <- c("ITS1", "ITS2", "SSU", "LSU", "5.8S")
  } else if ("none" %in% save_regions) {
    out.regions <- character(0)
  } else {
  ...
  for (r in out.regions) {
    ...
    if (isTRUE(do.partial)) {
      readfile <- paste0(out.root, ".full_and_partial.", r, ".fasta")
      if (file.exists(readfile)) {
  ...
  if (isTRUE(summary)) {
    readfile <- paste0(out.root, ".summary.txt")
    if (file.exists(readfile)) {
  ...
  if (isTRUE(graphical)) {
    readfile <- paste0(out.root, ".graph")
    if (file.exists(readfile)) {
  ...
  if (isTRUE(concat)) {
    readfile <- paste0(out.root, ".concat.fasta")
    if (file.exists(readfile)) {
  ...
  if (isTRUE(positions)) {
    readfile <- paste0(out.root, ".positions.txt")
    if (file.exists(readfile)) {
      out$positions <-
  ...
  if (isTRUE(table)) {
    readfile <- paste0(out.root, ".hmmer.table")
    if (file.exists(readfile)) {
  ...
  if (isTRUE(detailed_results)) {
    readfile <- paste0(out.root, ".extraction.results")
    if (file.exists(readfile)) {
  ...
  if (isTRUE(not_found)) {
    readfile <- paste0(out.root, "_no_detections.txt")
    if (file.exists(readfile)) {
